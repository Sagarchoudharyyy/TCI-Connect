...
async def create_case(
    ...
    try:                                                
        ...
        if (
            case.details
            and case.details.implant_details
        ):
        ...
        if not new_case:
            raise HTTPException(
                status_code=500,
                detail="Failed to reload created case"
            )
        ...
        new_case_payload = {
            ...
            "details": {
                ...
                "material_type":
                    ...
                    if case_detail
                    and case_detail.material_type
                    else [],

                "crown_bridge":
                    case_detail.crown_bridge.split(",")
                    if case_detail
                    and case_detail.crown_bridge
                    else [],

                "additional_restorations":
                    case_detail.additional_restorations.split(",")
                    if case_detail
                    and case_detail.additional_restorations
                    else [],
...
async def confirm_preview_files(
    case_id: int,
    db: Session = Depends(get_db)
):
    case = (
    ...
    db.refresh(case)
       ...
       # Send preview status update to Admin
    await manager.send_case_update(
        1,
        {
            "type": "preview_status_updated",
    ...
    # Send preview status update to Doctor
    await manager.send_case_update(
        case.doctor_id,
        {
            "type": "preview_status_updated",
